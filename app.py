...
STATE_STATS["people_per_salon"] = (
    (STATE_STATS["population"] / STATE_STATS["total_salons"]).round(0).astype(int)
)
...
ZIP_DATA["high_priority"] = (ZIP_DATA["pct_black"] > 0.30) | (
    ZIP_DATA["pct_nonwhite"] > 0.50
)
...
LOCALITY_DATA = LOCALITY_DATA.merge(
    STATE_STATS[["state", "people_per_salon"]].rename(
        columns={"people_per_salon": "benchmark"}
    ),
    on="state",
    how="left",
)
LOCALITY_DATA["people_per_shop"] = (
    LOCALITY_DATA["population"] / LOCALITY_DATA["scraped_shops"]
).round(1)
...
LOCALITY_DATA["coverage"] = LOCALITY_DATA["kpi"].apply(
    lambda k: (
        "Adequate"
        if k >= 1.0
        else "Near" if k >= 0.8 else "Under" if k >= 0.5 else "Severely Under"
    )
)
...
if page == "State Overview":
    ...
    filtered = (
        STATE_STATS
        if wave_filter == "All"
        else STATE_STATS[STATE_STATS["wave"] == wave_filter]
    )
    ...
    ax.set_title(
        "State Salon Density vs. National Average\nlower = more salons per capita",
        fontsize=11,
        pad=12,
    )
    ...
    nat = mpatches.Patch(
        color="#ff6b6b", label=f"National avg 1:{NATIONAL_BENCHMARK:.0f}"
    )
    ax.legend(
        handles=[wave1, wave2, nat],
        fontsize=12,
        facecolor="#1a1a1a",
        edgecolor="#333",
        labelcolor="#ddd",
    ...
    display_df["Population (2023)"] = display_df["Population (2023)"].apply(
        lambda x: f"{x:,}"
    )
    display_df["Total Salons"] = display_df["Total Salons"].apply(lambda x: f"{x:,}")
    display_df["People / Salon"] = display_df["People / Salon"].apply(
        lambda x: f"1:{x:,}"
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_in:
        ...
        population = st.number_input(
            "POPULATION", min_value=1000, max_value=10_000_000, value=28000, step=1000
        )
        scraped_shops = st.number_input(
            "SCRAPED SHOPS IN DATASET", min_value=1, max_value=500_000, value=45, step=5
        )

        benchmark = int(
            STATE_STATS.loc[
                STATE_STATS["state"] == state_sel, "people_per_salon"
            ].values[0]
        )
        total_state_salons = int(
            STATE_STATS.loc[STATE_STATS["state"] == state_sel, "total_salons"].values[0]
        )
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(
        "<div class='section-label'>How does this city compare?</div>",
        unsafe_allow_html=True,
    )
    ...
    if len(compare_data) > 0:
            ...
            [
                {
                    ...
                    "scraped_shops": scraped_shops,
                    "benchmark": benchmark,
                    "people_per_shop": people_per_shop,
                    "kpi": kpi,
                    "coverage": status.split("  ")[-1],
                }
            ]
        )
        compare_data = pd.concat(
            [compare_data, new_row], ignore_index=True
        ).sort_values("kpi")
        ...
        ax3.set_title(
            f"{state_sel} — Locality Coverage Comparison\n(white bar = your input)",
            fontsize=15,
            fontweight="bold",
        )
        ...
        st.info(
            "No comparison localities available for this state in the demo dataset."
        )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_f1:
        ...
        sel_states = st.multiselect(
            "STATE", states_available, default=["GA", "PA", "WI", "NH"]
        )
    with col_f2:
        priority_filter = st.radio(
            "PRIORITY", ["All", "High Priority Only", "Standard Only"]
        )
    with col_f3:
        income_max = st.slider(
            "MAX MEDIAN INCOME ($)", 20000, 100000, 100000, step=5000, format="$%d"
        )

    col_f4, col_f5 = st.columns(2)
    with col_f4:
        black_min = st.slider(
            "MIN % BLACK POPULATION", 0, 100, 0, step=5, format="%d%%"
        )
    with col_f5:
        nonwhite_min = st.slider(
            "MIN % NON-WHITE POPULATION", 0, 100, 0, step=5, format="%d%%"
        )
    ...
    if len(df) > 0:
        pivot = (
            df.groupby(["state", "high_priority"])
            .size()
        ...
        ax4.legend(
            facecolor="#1a1a1a", edgecolor="#333", fontsize=12, labelcolor="#ddd"
        )
    ...
    # Data table
    st.markdown(
        "<div class='section-label'>Zip Code Detail</div>", unsafe_allow_html=True
    )
    ...
    display["High Priority"] = display["High Priority"].apply(
